...
if(!list_empty(&ready_list)){
if(new_priority <= list_entry(list_begin(&ready_list), struct
...
void     /*添加方法check_if_blocked_thread*/
check_if_blocked_thread(struct thread *t,void *aux UNUSED)
{
...
{
thread_unblock(t);/*把线程放入就绪队列*/
}
}
}
bool
priority_less(const struct list_elem *e1,const struct list_elem *e2, void *aux){
return list_entry(e1, struct thread, elem)->priority > list_entry(e2, struct thread, elem)->priority;//优先级判断
}
